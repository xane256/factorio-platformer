...
Fork of two Factorio mods by Corlin (GPLv3), ported to Factorio 2.1:

- **`platformer/`** — [Platformer](https://mods.factorio.com/mod/platformer), a
  skyblock-style Space Age overhaul: you live on a single space platform, never
  land, and feed the factory from asteroids. Upstream 1.0.6 targeted 2.0.
- **`space-platform-chests/`** — [Space Platform Chests](https://mods.factorio.com/mod/space-platform-chests),
...
Mod names and prototype namespaces are unchanged, so these versions shadow the
upstream zips in a mods folder (Factorio loads the highest version per name)
and existing saves keep working. Publishing to the mod portal would need new
mod names — the upstream names belong to Corlin's account.
